...
Umbrella Review, según \textcite{aromataris2014umbrella}, es un tipo de revisión sistemática que recopila y analiza evidencia secundaria,
...
A través de la propagación de citaciones se amplía y actualiza la literatura encontrada en las bases de datos tradicionales.
De este modo, se superan limitaciones como la
...
\begin{enumerate}
    ...
    Se justifica en la necesidad de consolidar evidencia secundaria de calidad, aprovechando el enfoque de propagación de citaciones
...
Estos pasos están basados en el modelo clásico de DSR de \textcite{peffers2008design}, que vom Brocke adapta y expande en su guía.
En la Figura~\ref{fig: DSR} muestra el ciclo de DSR  dedicado a Centinela. La fila superior muestra las 6 fases
y los recuadros inferiores brindan mayor información acerca de cada fase. Además, el contenedor inferior resume tres puntos de entrada de investigación que guían las
...
at ([yshift=-0.75cm]current bounding box.south) {};
        ...
        \newcommand{\sep}{7} % Separación horizontal
...
\subsection{Ingesta} 
De acuerdo al estudio exhaustivo de la literatura, el primer paso para la construccion de RAG es la obtencion de los datos. En nuestro caso
...
de entrada más frecuentes para consultas y embeddings\parencite{zhai2024llmIR}.
De acuerdo con la literatura~\parencite{zhai2024llmIR,knollmeyer2024benchmarking,fan2021pretraining,ibrihich2022review,bernard2025fate,ma2025vector}, se han saleccionado los siguientes campos que
contienen la información necesaria para representar el contenido del documento e identificarlo.

\begin{itemize}
    \item \textbf{Título y abstract}: condensan la información temática central del artículo y son la base principal para la recuperación y la generación de embeddings.
...
vectorial.
Separar título/resumen de metadatos de control (DOI, ScopusID, autores, afiliaciones)  mejora la interpretabilidad del sistema y evita sobrecargar el espacio vectorial
con señales que no aportan significado textual directo.

El Algoritmo~\ref{alg:export-articles-csv} describe el proceso de extracción del corpus y su almacenamiento en un
archivo CSV (UTF-8) para su procesamiento en fases posteriores.
\begin{algorithmEN}
...
Una vez completada la ingesta de datos provenientes de Scopus, es fundamental preprocesarlos para normalizar y limpiar el texto, reducir ruido y mejorar la calidad semántica de las representaciones.
En esta etapa aplicaré el siguiente flujo reproducible compuesto por estos pasos:
\begin{enumerate}
...
El Algoritmo~\ref{alg:process-and-label} muestra el procedimiento para normalizar las columnas que contienen toda la semántica:
\begin{algorithmEN}
...
es indeterminado) y se guarda un nuevo Parquet.
La detección de idioma permite tratar corpus multilingües de forma óptima (stopwords/tokenización por lengua y elección del modelo de embeddings multilingüe).

 \item \textbf{Segmentación en oraciones:} El algoritmo~\ref{alg:segment-text-lang-aware}
    ilustra el proceso de segmentación sensible al idioma, aplicado sobre los textos previamente normalizados.
\begin{algorithmEN}
...
añadiendo el índice de oración sentence-idx dentro de cada documento y escribe el resultado en processed-sentences.parquet.
    ...
    Segmentar cada oración en tokens (palabras o subpalabras) preservando frases clave frecuentes (ej.,\textit{aprendizaje automático}), a fin de mantener unidades semánticas estables.
    El algoritmo~\ref{alg:tokenize} ilustra el proceso de tokenización aplicado a las oraciones segmentadas.
  \begin{algorithmEN}[H]
  \caption{Tokenize} 
  \begin{algorithmic}[1] 
...
Luego aplica ambos phrasers para unir collocations frecuentes en un solo token el cual a su vez genera tokens. Los tokens son unidos por comas y se guardan en el nuevo parquet.
    ...
    \item \textbf{Eliminación de stop words:} Consiste en remover términos de alta frecuencia y escaso valor semántico.
El algoritmo~\ref{alg:remove_stopwords_lang} describe el proceso de eliminación de \textit{stop words} adaptado al idioma de cada documento.
\begin{algorithmEN}[H]
...
\item \textbf{Aplicación de Lematización o stemming:} Se prefiere la lematización frente al stemming para conservar la forma canónica de las palabras y reducir la varianza léxica sin pérdida de significado \parencite{tabassum2020}.
    El algoritmo~\ref{alg:lemmatize-spacy} muestra el proceso de lematización multilingüe aplicado al corpus depurado,
    empleando modelos lingüísticos específicos para español e inglés con el fin de unificar las variantes morfológicas
    y optimizar la coherencia semántica del texto para su posterior representación vectorial.
\begin{algorithmEN}[H]
...
\item \textbf{\textit{Chunking} para LLM/RAG:} Consiste dividir documentos largos en fragmentos (\(\sim 200\text{--}400\) tokens) con solape (15–30\,\%) para respetar
    límites de contexto de los transformadores y mejorar la recuperación a nivel de fragmentos de texto.
    El algoritmo~\ref{alg:chunking} muestra el procedimiento empleado para segmentar los textos en fragmentos
    parcialmente superpuestos, preservando la continuidad del contenido y optimizando la detección de evidencia
    relevante en las fases de recuperación y generación del sistema RAG.
...
Para el diseño de este sistema se usa un Recuperador denso de arquitectura bi-encoder tipo BERT para la generación de embeddings, ya que permite codificar por separado la 
...
\begin{algorithmEN}[H]
  ...
  \State $meta\_\min \gets \textsc{Select}(\texttt{chunks\_df},\ cols)$
...
El Algoritmo~\ref{alg:build_vector_index} describe cómo construir un índice vectorial (FAISS) a partir de una matriz de embeddings previamente generada.
\begin{algorithmEN}[H]
...
El Algoritmo~\ref{alg:search_vector_index} describe el proceso de \textbf{búsqueda y recuperación de información} utilizando el índice
vectorial (FAISS) y los metadatos generados en el paso anterior.
\begin{algorithmEN}[H]
...
El Algoritmo~\ref{alg:re-ranking} implementa un cross encoder entrenado de segunda capa que reordena los candidatos recuperados en la primera etapa densa. El puntaje final se obtiene por fusión lineal (min-max + suma ponderada)
...
El Algoritmo~\ref{alg:build-context-blocks} procesa los k mejores documentos para crear una lista estructurada de bloques de contexto.
\begin{algorithmEN}[H]
...
\end{algorithmic}
    \label{alg:build-context-blocks}
\end{algorithmEN}
...
El Algoritmo~\ref{alg:llm-generate-via-http} recibe un prompt estructurado que incluye la instrucción de estilo y formato
...
\end{algorithmic}
    \label{alg:llm-generate-via-http}
\end{algorithmEN}
...
\begin{itemize}[leftmargin=*, itemsep=0.25em]
    \item \textit{Recuperador}: Cobertura y ranking.
...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
    \item \textbf{LLM-solo (sin Recuperador):} establece la \emph{línea base}.
    \item \textbf{RAG-básico:} Recuperador + \emph{concatenación} de los documentos \emph{top-\(k\)} (sin reranqueo) + generación.
    \item \textbf{RAG-completo:} Recuperador +  \emph{reranking} + \emph{construcción de bloques de contexto} + generación.
\end{itemize}
La Tabla~\ref{tab:comparativa} presenta una comparación entre las tres configuraciones principales evaluadas en este trabajo: LLM-solo, RAG-básico y RAG-completo.
En ella se resumen los componentes activos y la estrategia de integración utilizada en cada caso.
\begin{table}[ht]
    \centering
    \renewcommand{\arraystretch}{1.1}
    \begin{tabular}{@{}lccccc@{}}
        \toprule
        \textbf{Condición} & \textbf{Recuperador} & \textbf{Reranker} & \textbf{Contexto} & \textbf{Estrategia de contexto} & \textbf{Generación} \\
        \midrule
        LLM-solo        & --          & --          & --          & --                       & \checkmark \\
        RAG-básico      & \checkmark  & --          & \checkmark  & Concatenación top-\(k\)  & \checkmark \\
        RAG-completo    & \checkmark  & \checkmark   & \checkmark  & Context Builder        & \checkmark \\
        \bottomrule
    \end{tabular}
    \caption{Comparativa entre las tres configuraciones del sistema.}
...
\begin{enumerate}[label=\textbf{(\roman*)},leftmargin=*,itemsep=0.35em]
    ...
    \item \textit{Recuperador}: se emplea \textbf{Recall@k} (con \(k=15\)) para medir la proporción de evidencia
    relevante incluida en el top-\(k\) (cobertura), y \textbf{MRR} para evaluar la posición promedio del primer documento
    relevante (sensibilidad al ranking).
    Ambas métricas se reportan como promedios sobre las consultas.

    \item \textit{Augmentation}: se reporta la \textbf{Ganancia de augmentation}, definida como la mejora en la métrica
    objetiva al inyectar contexto frente al uso del LLM sin recuperación,
    \(\Delta\mathcal{M}=\mathcal{M}(\text{RAG})-\mathcal{M}(\text{LLM}_{\text{solo}})\).
    Esta diferencia refleja el valor añadido por la construcción del contexto.

    \item \textit{Generation}: dado que todas las consultas son de tipo abierto, la evaluación se realiza mediante un
    juez automático (\emph{LLM-as-Judge}, GPT~4.1), siguiendo las recomendaciones metodológicas de \textcite{wu2025ragSurvey,han2024rag}.
    El objetivo es valorar la calidad semántica y factual de las respuestas generadas, considerando tanto
    su coherencia con el contexto recuperado como su concordancia con la respuesta ideal (\emph{ground truth}).
    A continuación, se va a explicar el protocolo utilizado para evaluar las métricas en la etapa de generación.
    \subparagraph{Evaluación de fidelidad al contexto.}
    En una primera etapa, el juez analiza la respuesta generada con base en los fragmentos de evidencia recuperados,
    evaluando:
    \begin{itemize}[leftmargin=*,itemsep=0.25em]
        \item \textbf{Fidelidad (Faithfulness):} cada afirmación de la respuesta debe estar lógicamente derivada
        del contexto, sin contradicciones ni invenciones.
        \item \textbf{Cobertura de citas (Citation Coverage):} proporción de afirmaciones sustentadas con una cita
        válida al contexto.
    \end{itemize}
    La \textbf{tasa de alucinación} se define como \(1 - \text{Fidelidad}\).
    El prompt exacto empleado por el juez para esta fase se presenta en el Anexo~\ref{ch:prompt_juez}.

    \subparagraph{Evaluación del acuerdo con la respuesta ideal.}
    En una segunda etapa, el juez compara la respuesta generada con la respuesta ideal definida en el ground truth,
    con el fin de medir su acuerdo semántico y factual.
    El protocolo seguido es el siguiente:
    \begin{enumerate}[label=\textbf{P\arabic*},leftmargin=*,itemsep=0.25em]
        \item \textbf{Preparación de datos:} se dispone, para cada consulta, de la pregunta,
        la respuesta ideal (\emph{gold answer}) y la respuesta generada por el sistema (\emph{RAG output}),
        almacenadas en un archivo CSV.
        \item \textbf{Entrada al juez:} para cada par (\emph{gold}, \emph{RAG}), el modelo recibe un prompt que incluye
        la pregunta, ambas respuestas y las instrucciones de comparación.
        Se le solicita emitir un veredicto estructurado en formato JSON con puntuaciones en una escala de 0 a 5 en
        las siguientes dimensiones:
        \begin{enumerate}[label=\alph*),leftmargin=*,itemsep=0.15em]
            \item \textbf{Exactitud factual:} coincidencia de hechos y ausencia de invenciones o distorsiones.
            \item \textbf{Cobertura semántica:} inclusión de la información esencial de la respuesta ideal.
            \item \textbf{Coherencia expresiva:} preservación del significado global, aunque la redacción difiera.
        \end{enumerate}
        \item \textbf{Repetición y control de variabilidad:} cada comparación se ejecuta tres veces con distinta
        semilla, y las puntuaciones se promedian para reducir la varianza inherente al modelo evaluador~\parencite{casola2022pretrained}.
        \item \textbf{Cálculo de métricas:} las tres puntuaciones asignadas por el juez (exactitud, cobertura y coherencia) se promedian para cada consulta, obteniendo un \textbf{índice global de acuerdo} (\(A_i\)) que resume el nivel general de coincidencia entre la respuesta RAG y la respuesta ideal:
        \[
            A_i = \frac{\text{Exactitud}_i + \text{Cobertura}_i + \text{Coherencia}_i}{3}.
        \]
        Posteriormente, se calcula el promedio macro \(\overline{A}\) sobre todas las consultas del conjunto de prueba.
        Para estimar la consistencia del sistema, las puntuaciones se transforman en valores
        binarios~\footnote{Consiste en transformar las puntuaciones continuas en categorías binarias de
        acuerdo (1) o desacuerdo (0), utilizando un umbral definido según la escala de evaluación empleada.},
        considerando el acuerdo cuando \(A_i \ge 4\) y desacuerdo en caso contrario.
        Con esta codificación se calcula el \textbf{coeficiente de acuerdo de Cohen} (\(\kappa\))~\footnote{Mide el grado
        de coincidencia entre dos evaluaciones. Valores cercanos a 1 indican alta concordancia y cercanos a 0 poca o ninguna.},
        que cuantifica la concordancia entre las evaluaciones más allá del azar.
        \item \textbf{Interpretación:} valores altos de \(\overline{A}\) y \(\kappa\) indican mayor
        proximidad semántica y factual de las respuestas generadas al ground truth, reflejando mejor desempeño del
        generador y mayor coherencia entre configuraciones experimentales.
    \end{enumerate}
    Este procedimiento permite evaluar de forma integral la generación, combinando fidelidad al contexto y acuerdo
    con la respuesta ideal, lo que ofrece una medida robusta de la calidad global del sistema.

    \item \textit{Operación}: se mide la \textbf{Latencia (Response Time)} por consulta mediante cronometraje de
    la ejecución \emph{end-to-end} (recuperación, reranqueo, construcción de contexto y generación).
