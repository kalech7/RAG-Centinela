...
Se eligen tres áreas temáticas del corpus y, en cada una, se ejecutan 50 consultas.
Las consultas se organizan en tres tipos: \emph{single-hop} (la respuesta está en un solo fragmento), \emph{multi-hop}
(requiere fusionar 2--3 evidencias) y \emph{no answer} (no existe respuesta en el corpus).
Las respuestas generadas se validan con apoyo de expertos.
\begin{enumerate}[label=\textbf{(\roman*)},leftmargin=*,itemsep=0.35em]
    \item \textit{Retriever}: usamos \textbf{Recall@k} (con \(k=20\)) para medir cuánta evidencia relevante entra
    ...
    \textbf{Cobertura de citas} (porcentaje de afirmaciones sustentadas).
    Además, reportamos la \textbf{Tasa de alucinación} como \(1-\)fidelidad.
    \item \textit{Operación}: medimos \textbf{Latencia (Response Time)} por consulta.
\end{enumerate}
