...
Los tipos de augmentation, \textcite{zhao2024rag} señalan que se puede aplicar en tres momentos distintos. En el pre-training, se integran representaciones estructuradas 
...
El ajuste del recuperador/reranker consiste en optimizar, con datos del dominio, el recuperador para maximizar el recall@k sustituyendo el objetivo de alineación por métodos
 como REPLUG y UPRISE: REPLUG trata al LLM como “caja negra” y ajusta el recuperador para que su distribución sobre documentos recuperados se acerque a la que induce el propio
  LLM, entrenando con una pérdida de divergencia KL 
\[
...
\]
UPRISE, por su parte, UPRISE entrena un prompt retriever usando a un LLM congelado como supervisor: el LLM puntúa qué prompts (de un banco de plantillas) 
...
en inferencia y sin reentrenar el LLM, los prompts más útiles en zero-shot\footnote{Resolver una tarea sin ejemplos etiquetados de esa tarea durante el ajuste}, elevando la calidad del contexto que finalmente consume el generador \textcite{zhao2024rag,fan2024ragllm}.
