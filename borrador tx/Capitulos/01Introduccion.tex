...
En los últimos años, la evolución de los modelos de lenguaje de gran escala (Large Language Models, LLM) ha
redefinido el procesamiento del lenguaje natural
...
contextualizadas.
En respuesta a esta limitación surge el enfoque de Retrieval-Augmented Generation (RAG), el cual combina la recuperación
de información con la generación de
lenguaje natural, logrando mejorar la precisión, la coherencia y la actualidad de las respuestas producidas por los
modelos.
Dada la creciente relevancia de los LLM, resulta necesario llevar a cabo una revisión exhaustiva de la literatura que
permita consolidar los avances recientes y evaluar los desafíos aún presentes.
Para ello, esta revisión se apoya en Umbrella SLR y propagación de citaciones usando el protocolo propuesto en la
Sección~\ref{sec:revision-sistematica}.
...
Para ello, el proceso de revisión se organiza en las fases que se presentan a continuación:
...
\end{itemize}
Estas fases buscan garantizar la consistencia, validez y pertinencia de la evidencia obtenida.
...
evaluación, almacenamiento en bases vectoriales y generación de resultados.
Asimismo, se examinan los paradigmas, las métricas de evaluación y el futuro de RAG.
...
científica en el área comenzó a incrementarse a partir de 2018.
Con base en información de
Lens.org~\footnote{Es una plataforma abierta
para la búsqueda, análisis y visualización de literatura científica y patentes.
Accesible en: \href{https://www.lens.org/}{Lens.org}}, este incremento coincide
...
un experto.
Se da preferencia a aquellos que presenten una cobertura amplia de los temas más relevantes para el objeto de estudio.
...
% para resume la información se uso (herramienta) atlas por ejemplo
...
las características y las aplicaciones, como se muestra en la Figura.~\ref{fig:Fudamentos}.
Posteriormente, se profundiza en la arquitectura, Figura. \ref{fig:arquitectura_rag},
...
En la Figura.~\ref{fig:paradigmas}, se examinan los paradigmas de RAG, presentando sus tipos y clases; seguidamente, se introducen las métricas y evaluadores automáticos
...
\begin{enumerate}
    ...
    \item \textbf{Aportes en evaluación}: se han desarrollado métricas específicas para auditar la calidad de un pipeline RAG (ej., context relevance, faithfulness, answer correctness, citation quality), hoy ya sistematizadas en la literatura \parencite{knollmeyer2024benchmarking}.
\end{enumerate}
...
RAG se compone de tres fases: recuperación, aumentación y generación (ver Figura~\ref{fig:arquitectura_rag}).
Como lo menciona \textcite{gao2023rag}, primero se
localizan documentos relevantes para la consulta (Recuperación); luego, se enriquece la entrada del usuario con esos textos (aumentación); y finalmente, el modelo produce
una respuesta basada tanto en su conocimiento interno como en la información recuperada (Generación).
Gracias a este enfoque, RAG incrementa la exactitud
...
cita de fuentes.
Se trata de una de las técnicas más relevantes en tareas que requieren una gran
cantidad de conocimiento, como en los ámbitos médico, legal o de investigación científica.
La Tabla~\ref{tab:retriver} resume el componente recuperación en tres categorías:
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Indexación}: incluye bases vectoriales y algoritmos de búsqueda.
    \item \textbf{Enhancements}: conjunto de técnicas que mejoran la calidad del contexto recuperado y su ordenación.
    \item \textbf{Tipos de retrievers}: abarcan enfoques \textit{sparse}, \textit{dense} e híbridos.
\end{enumerate}
...
\midrule
Indexación    & Vector DB               & FAISS, Annoy, Milvus, Weaviate, Pinecone, Qdrant\\
...
\paragraph{Indexación:} Es el encargado de organizar y representar la información de forma eficiente.
En este sentido, los vector databases (VDBs) han aumentado su popularidad, ya que permiten almacenar
vectores de alta dimensionalidad y realizar búsquedas por similitud semántica.
Como explica~\textcite{joshi2025vector}, estas bases de datos resultan esenciales para
...
recuperador base.
De acuerdo con la Tabla~\ref{tab:enhancementstechnics}, resume un conjunto de técnicas de mejora del pipeline RAG:
...
De acuerdo con la literatura \parencite{hambarde2023ir,fan2021pretraining,han2024rag,wu2025ragSurvey}, estas categorías agrupan las técnicas más empleadas y reportadas en revisiones exhaustivas de la literatura.
No obstante,
...
Los dense usan representaciones continuas aprendidas (embeddings) para medir similitud semántica, capturando sinonimia y paráfrasis con mejor recall semántico, aunque
 con mayor costo de indexación y búsqueda.
 Los enfoques híbridos/otros combinan señales léxicas y semánticas
...
A continuación, se listan los diferentes tipos de retrievers.
% Lista de retrievers por categoría (sparse, dense, híbridos/otros)
\begin{itemize}
  \item \textbf{Sparse}
    \begin{itemize}
      \item \textbf{BM25}: Modelo probabilístico que pondera frecuencia de términos, IDF y normaliza por longitud.
      A diferencia de otros enfoques dispersos, BM25 introduce una función de saturación que evita que la alta repetición de un término en un documento incremente indefinidamente su peso. Es la línea base más usada como primer filtro eficiente en RAG y RI a gran escala \textcite{hambarde2023ir,han2024rag}.
...
La Tabla~\ref{tab:aumentación} contiene tres ejes del componente aumentación: Tipos, Data y Process. Los cuales enriquecen el contexto disponible y mejoran la pertinencia
de la recuperación.
...
\midrule
Process & Once                  & document aumentación \\
        & Iterative             & pseudo-relevance feedback (PRF) \\
        & Adaptative            & SKR (Selective Knowledge Retrieval)\\
\bottomrule
\end{tabularx}
\caption{Componente aumentación}
\label{tab:aumentación}
\end{table}

Aumentación es el proceso el cual un modelo de lenguaje incorpora información adicional
...
\paragraph{Tipos de aumentación:} \textcite{zhao2024rag} señalan que se puede aplicar en tres momentos distintos. En el pre-training, se integran representaciones estructuradas 
...
lo que impacta la selección de modelo y la fiabilidad de la evaluación.
...
puede anteponer vectores al input.
Asi, cada capa contiene el contexto que dirige la atención del modelo hacia patrones deseados, manteniendo intactos los pesos base del modelo.
...
De este modo, el recuperador aprende a imitar las preferencias del LLM respecto a qué documentos son más relevantes,
logrando así seleccionar de manera más efectiva aquellos fragmentos que aportan información útil.
Este alineamiento mejora el \textit{recall@k} y asegura que el contexto
...
\paragraph{Datos para la fase de aumentación:} \textcite{fan2024ragllm} destacan tres clases. La información estructurada, como los Knowledge Graphs, es fundamental para tareas de razonamiento
...
\paragraph{Procesos de aumentación:} Se clasifican en tres modalidades según~\textcite{zhao2024rag}. 
aumentación puede aplicarse una sola vez (Once), como en el caso de la document aumentación, 
...
más fiables, contextualizadas y relevantes~\parencite{casola2022pretrained,fan2024ragllm,zhang2025hallucination}.
...
Con respecto a la mejora, el Prompt Engineering ha demostrado ser una técnica central para guiar los modelos hacia respuestas más precisas y controladas.
Según \textcite{zhang2025hallucination},
...
que permiten modular el balance entre coherencia y diversidad.
Entre ellas, el beam search explora varias rutas posibles de generación en paralelo para seleccionar
la más prometedora, garantizando coherencia aunque sacrificando creatividad.
Por su parte, el nucleus sampling restringe las opciones a un conjunto dinámico de palabras
...
\subsubsection{Fases de Implementación de los sistemas RAG}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm]
  \def\step{-1.5}
...
vectoriales. \textcite{knollmeyer2024benchmarking}
...
La evaluación en sistemas RAG debe abordarse de forma multifásica, verificando tanto la recuperación (calidad y suficiencia del contexto) como la generación
 (fidelidad y atribución), ya que el desempeño global depende de ambas etapas del pipeline~\parencite{knollmeyer2024benchmarking}.
\mbox{}\\
...
El proceso de evaluación, sus fases específicas y las métricas asociadas se describen con mayor detalle en la
Sección~\ref{subsubsec:metricas}, donde se resumen las métricas clásicas de recuperación,
generación y fidelidad, así como los indicadores operativos y criterios FATE
que complementan la evaluación integral del sistema.

\subsubsection{Paradigmas RAG}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm]
\tikzset{
...
% ================== Parámetros  ==================
\newcommand{\sideW}{4.5cm}  % MISMO ancho para ambos hijos de nivel 1
\newcommand{\sideX}{4.0}    % desplazamiento horizontal simétrico de hijos
\newcommand{\sideY}{2.7}    % desplazamiento vertical de hijos
\newcommand{\suby}{-2.0}    % caída vertical de subnodos de RAG
\newcommand{\dx}{3.8}       % separación horizontal de subnodos de RAG
\newcommand{\childShift}{-1.5}
...
\subsubsection{Evaluación y Métricas}\label{subsubsec:metricas}
...
de la respuesta, corrección y calidad de las citas.
Estas dimensiones permiten evaluar de manera más integral los sistemas que combinan recuperación y generación.
Asimismo,
...
\begin{itemize}
  \item \textbf{Enfoques de Evaluación}
        \begin{itemize}
          \item Evaluación independiente por etapas: análisis separado de retrieval, aumentación y generation, con métricas específicas en cada módulo.
          \item Evaluación End-to-End: valoración directa de la salida final del sistema RAG, con dos variantes:
          \begin{itemize}
              \item Evaluación End-to-End automática: consiste en medir la calidad final de un sistema RAG sobre su salida
              completa y verificándolas automáticamente frente al contexto recuperado y/o Ground Truth~\footnote{\textbf{Ground Truth:} Es el conjunto
              de respuestas correctas ya establecidas normalmente creadas por personas expertas o tomadas de fuentes verificadas,
                  se usa para comparar y medir qué tan bien acierta un sistema} mediante evaluadores como NLI
              (\textit{entailment})~\footnote{\textbf{Entailment (NLI):} significa que una una afirmación/claim de la respuesta se sigue o está sostenida
              por un fragmento de texto recuperado sin contradicciones. Cuando no hay soporte es \emph{neutral} y cuando la niega es \emph{contradiction}}
              o \textit{LLM-as-judge} (un LLM que actúa como evaluador automático de la respuesta)
      ...
      \end{itemize}
    \item \textbf{Evaluación en IR con FATE}
    \begin{itemize}
    ...
    \item \textbf{Evaluación Clásica en IR}
    \begin{itemize}
...
Este glosario permite la comprensión de los conceptos evaluados, la notación usada y los cálculos asociados a cada
métrica.
\begin{table}[H]
    \centering
    \begin{tabularx}{\textwidth}{L{0.22\textwidth} Y}
        \toprule
        \textbf{Símbolo/Sigla} & \textbf{Significado} \\
        \midrule
        TP, FP, FN, TN & Verdaderos/Falsos positivos y negativos. \\
        $\displaystyle N$ & Número total de ejemplos/instancias evaluadas. \\
        $\displaystyle Q$ & Conjunto de preguntas o consultas; $\lvert Q\rvert$ su cardinalidad. \\
        $\displaystyle \lvert A\rvert$ & Cardinalidad del conjunto $A$. \\
        $\displaystyle \mathrm{Rel}$ & Conjunto de documentos relevantes (ground truth de relevancia). \\
        $\displaystyle \mathrm{Top}\!-\!k$ & Conjunto de los $k$ documentos mejor clasificados/recuperados. \\
        $\displaystyle k$ & Tamaño del corte en \emph{Top-$k$}. \\
        $\displaystyle \mathrm{rank}_i$ & Posición del primer documento relevante para la consulta $i$. \\
        $\displaystyle \mathcal{M}(\cdot)$ & Métrica evaluada (p.\,ej., \textit{Accuracy}, F1, EM, etc.). \\
        $\displaystyle \hat{y}_i$ & Respuesta generada por el modelo para el ejemplo $i$. \\
        $\displaystyle Y_i^{\star}$ & Conjunto de respuestas de referencia (\textit{ground truth}) para el ejemplo $i$. \\
        $\displaystyle \mathbf{1}\{\cdot\}$ & Función indicadora (vale 1 si la condición se cumple, 0 si no). \\
        $\displaystyle \mathrm{score}_i$ & Puntuación (p.\,ej., de fidelidad/consistencia) asignada a la respuesta $i$ por un juez (LLM o clasificador). \\
        $t^{(\text{cliente})}_{\text{inicio},j}$, $t^{(\text{cliente})}_{\text{fin},j}$ & Tiempos de inicio y fin (en el cliente) de la consulta $j$. \\
        $T_j$ & Latencia de la consulta $j$: $T_j = t^{(\text{cliente})}_{\text{fin},j}-t^{(\text{cliente})}_{\text{inicio},j}$. \\
        $j$ & Índice de la consulta en la evaluación operacional. \\
        \#\,X & Número de elementos de $X$ (p.\,ej., \#\,\textit{claims}, \#\,\textit{chunks}). \\
        \textit{chunk} & Fragmento/segmento de documento recuperado. \\
        \textit{claim} & Afirmación atómica evaluable dentro de una respuesta. \\
        $\mathrm{QPS}$ & \textit{Queries per second} (throughput). \\
        $\mathrm{Cost}$ & Costo monetario por componente del pipeline. \\
        \bottomrule
    \end{tabularx}
    \caption{Glosario de notación}
...
\begin{tabularx}{\textwidth}{L{0.22\textwidth} Y C{0.30\textwidth} C{0.16\textwidth}}
    \toprule
    \textbf{Métrica} & \textbf{Descripción} & \textbf{Fórmula} & \textbf{Propósito} \\
    \midrule

    % --- Recuperación
    \multicolumn{4}{l}{\textbf{Recuperación}}\\
    \cmidrule(lr){1-4}
    Recall@k & Proporción de documentos relevantes capturados dentro del top-$k$. &
    $\displaystyle \frac{\lvert \mathrm{Rel}\cap \mathrm{Top}\!-\!k\rvert}{\lvert \mathrm{Rel}\rvert}$ & eficacia \\[4pt]
    Precision & Fracción de ítems recuperados que son relevantes. &
    ...
    F1 & Media armónica entre \textit{precision} y \textit{recall}. &
    $\displaystyle \frac{2\,\mathrm{Precision}\cdot \mathrm{Recall}}{\mathrm{Precision}+\mathrm{Recall}}$ & equilibrio \\[4pt]
    Relevancia de contexto (clasificador/reranker) & \% de fragmentos recuperados juzgados relevantes automáticamente. &
    $\displaystyle \frac{\#\,\text{chunks relevantes}}{\#\,\text{chunks evaluados}}$ & pertinencia \\[4pt]

% --- aumentación
    \addlinespace[2pt]
    \multicolumn{4}{l}{\textbf{Aumentación}}\\
    \cmidrule(lr){1-4}
    Ganancia de aumentación & Efecto marginal de inyectar contexto frente a usar el LLM sin recuperación. &
    $\displaystyle \mathcal{M}(\text{RAG}) - \mathcal{M}(\text{LLM}_{\text{sin retriever}})$ & mejora \\[4pt]

% --- Ranking
    \addlinespace[2pt]
    \multicolumn{4}{l}{\textbf{Ranking}}\\
    \cmidrule(lr){1-4}
    MRR (Mean Reciprocal Rank) & Evalúa la posición del primer relevante en el ranking. &
    $\displaystyle \frac{1}{\lvert Q\rvert}\sum_{i=1}^{\lvert Q\rvert}\frac{1}{\mathrm{rank}_i}$ & ranking \\[4pt]

% --- Generación
    \addlinespace[2pt]
    \multicolumn{4}{l}{\textbf{Generación}}\\
    \cmidrule(lr){1-4}
    Accuracy & Proporción total de aciertos en verificación binaria. &
    $\displaystyle \frac{TP+TN}{N}$ & exactitud \\[4pt]
    Error rate & Complemento de \textit{accuracy}. &
    $\displaystyle \frac{FP+FN}{N}$ & error \\[4pt]
    Exact Match (EM) &  de respuestas que coinciden exactamente con la referencia. &
    $\displaystyle \frac{1}{N}\sum_{i=1}^{N}\mathbf{1}\{\hat{y}_i\in Y_i^{\star}\}$ & coincidencia \\[4pt]
    Cobertura de citas &  de afirmaciones sustentadas con pasajes citados. &
    $\displaystyle \frac{\#\,\text{afirmaciones sustentadas}}{\#\,\text{afirmaciones totales}}$ & atribución \\[4pt]
    Fidelidad (faithfulness) &  de afirmaciones consistentes con las fuentes recuperadas. &
    $\displaystyle \frac{\#\,\text{afirmaciones consistentes}}{\#\,\text{afirmaciones evaluadas}}$ & fidelidad \\[4pt]
    LLM-as-judge (fidelidad) & Puntuación de consistencia afirmación–evidencia; promedio por respuesta. &
    $\displaystyle \frac{1}{N}\sum_{i=1}^{N}\mathrm{score}_i$ & fidelidad \\[4pt]
    NLI (entailment a nivel de afirmación) & Porcentaje de afirmaciones \emph{entailed} por los pasajes recuperados. &
    $\displaystyle \frac{\#\,\text{claims entailed}}{\#\,\text{claims totales}}$ & veracidad \\[4pt]
    Auto-consistency & Acuerdo entre múltiples generaciones sobre hechos clave. &
    $\displaystyle \frac{\#\,\text{acuerdos}}{\#\,\text{comparaciones}}$ & robustez \\[4pt]
    Tasa de alucinación & Porcentaje de afirmaciones sin soporte en el contexto recuperado. &
    $\displaystyle 1 - \text{fidelidad}$ & riesgo \\[4pt]

% --- Operación
    \addlinespace[2pt]
    \multicolumn{4}{l}{\textbf{Operación}}\\
    \cmidrule(lr){1-4}
    Latencia (Response Time) & Tiempo por consulta (cliente: fin − inicio). &
    $\displaystyle T_j = t^{(\text{cliente})}_{\text{fin},j}-t^{(\text{cliente})}_{\text{inicio},j}$ & rapidez \\[4pt]
    Throughput (QPS) & Consultas servidas por segundo. &
    $\displaystyle \frac{\#\,\text{consultas}}{\text{segundos}}$ & capacidad \\[4pt]
    Costo por consulta & Costo monetario unitario de servir una consulta. &
...
Debido al poder de otorgar documentos como evidencia, aportando contexto y trazabilidad de las fuentes, RAG se consolidará como una capa estructural de acceso conversacional
 al conocimiento, donde LLM e IR cooperan para entregar respuestas verificables y reducir alucinaciones.
 De esta forma, avances como la recuperación jerárquica y el uso de bases de datos vectoriales
...
\mbox{}\\
Los sistemas RAG enfrentan varios retos clave que afectan su adopción y desempeño en contextos críticos:
\begin{itemize}
    \item Generación de alucinaciones, comprometiendo la confianza en los resultados~\textcite{zhai2024llmIR}.
    \item Limitaciones en la calidad de la recuperación y la eficiencia computacional, sobre todo en dominios especializados~\parencite{hu2024ragrau}.
    \item Dificultades de escalabilidad y adaptación al integrar RAG con otras arquitecturas~\textcite{ramdurai2025llm}.
    \item Problemas persistentes de ética y transparencia (FATE).
    \item Ausencia de marcos estandarizados para evaluar imparcialidad y responsabilidad, lo que restringe la confianza social~\textcite{bernard2025fate}.
    \item Reto de reproducibilidad y robustez debido a la variabilidad en hiperparámetros y semillas~\parencite{fan2021pretraining}.
\end{itemize}
Abordar estos desafíos es permitirán avanzar en la fiabilidad, equidad y aplicabilidad de los sistemas RAG en
escenarios sensibles y de alto impacto.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \path[
            mm/base,
            level 1/.append style=mm/level1
        ]
        node[mm/root] {Direcciones}
        child { node[mm/child]     {Retrievers\\\& bucle modelo--RI} }
        child { node[mm/childAlt]  {Multimodal\\} }
        child { node[mm/child]     {RAPTOR\\} }
        child { node[mm/childAlt]  {VDB\\memoria\\} }
        child { node[mm/child]     {Transformers\\en IR\\} };
    \end{tikzpicture}
    \caption{Principales direcciones}
    \label{fig:mindmap-direcciones}
\end{figure}
Como se muestra en la Figura~\ref{fig:mindmap-direcciones}, las líneas de avance incluyen el desarrollo de retrievers más robustos, la optimización de las interacciones entre modelo y recuperación,
...
capaces de combinar información textual, visual y estructurada.
Asimismo, enfoques como RAPTOR, que introduce resúmenes recursivos y jerárquicos en la recuperación,
muestran cómo superar la fragmentación del contexto y mejorar la integración semántica~\parencite{sarthi2024raptor}.
Otra dirección destacada es la incorporación de
...
aunque todavía requieren diseños de pre entrenamiento adaptados a los desafíos específicos de la recuperación de información~\parencite{fan2021pretraining}.
...
pero con respaldo en documentación verificable, capaces de planificar, citar y aprovechar el historial conversacional.
Incluso si los LLM siguen mejorando, la recuperación seguirá
...
herramientas externas y módulos memoria trabajen de forma coordinada.
Esta coordinación hará posible planificar y ejecutar tareas más largas o compuestas.
La evaluación de estos sistemas ya no se limitará a medir métricas técnicas como la exactitud, sino que pondrá el énfasis en
